...
class GoodRead {
     ...
     * @param string $query
     * @param integer $page 
     * @param string $field title|author|all
     */
    public function search($query, $page = 1, $field = "all")
    {
        $validFields = array("title", "author", "all");
        if (!in_array($field, $validFields)) {
            throw new Exception("Invalid search field, it should be one of title|author|all");
        }
        ...
        return $this->request(
            ...
            array(
                'q' => $query,
                'page' => (int)$page,     // :( GoodReads still doesn't support JSON for this endpoint
                'key' => $this->apiKey,
                'search[field]' => $field
            )
        );
